...
public class EnteringSexCallbackHandler : ICallbackHandler
{
    ...
    public EnteringSexCallbackHandler(ICallbackGenerator callbackGenerator,
    ...
    private async Task OnChoosingSexAsync(Sex sex, ITelegramBotClient botClient, User userFrom,
                                          CancellationToken cancellationToken)
    {
        ...
        UserRegistrationContract contract = new(state.UserId, state.Gender, state.Age, state.RestorationStep);
        BusinessUser? user = await _userRegistrationService.RegisterUserAsync(contract, cancellationToken);
        if (user == null) throw new Exception($"Failed to register user {userFrom.Id}.");

        const string responseOnSuccessfulRegistration = """
                                                        Вы были успешно зарегистрированы.
                                                        """;
        await botClient.SendMessage(userFrom.Id, responseOnSuccessfulRegistration,
            cancellationToken: cancellationToken);
    }
}